// Self-issued, signed session cookie for the db_tbh_user login. Independent of
// hrkpisSession.ts (which reads the legacy PHP session file) - see session/route.ts
// for how the two are combined.
import { createHmac, timingSafeEqual } from "crypto";
import type { NextResponse } from "next/server";
import { APP_SESSION_COOKIE_NAME } from "@/src/lib/server/sessionCookieNames";

export type AppSessionPayload = { userInv: string; empId?: string };

const SESSION_COOKIE_NAME = APP_SESSION_COOKIE_NAME;
const SESSION_SECRET = process.env.AUTH_SESSION_SECRET || "";
const AUTH_COOKIE_SECURE = process.env.AUTH_COOKIE_SECURE;
// Idle timeout: the session ends after 30 minutes without activity. Every renewal
// (see renewCookie) pushes the expiry 30 minutes ahead again. The expiry is stored inside
// the signed payload, so the server enforces it and does not rely on the browser
// dropping the cookie. Not logged in => back to /login (see proxy.ts).
export const SESSION_IDLE_SECONDS = 30 * 60;

export class AuthSessionService {
  constructor(private readonly now: () => number = Date.now) {}

  private isSecureCookie() {
    if (AUTH_COOKIE_SECURE !== undefined) return AUTH_COOKIE_SECURE !== "false";
    return process.env.NODE_ENV === "production";
  }

  private getSigningSecret() {
    if (SESSION_SECRET) return SESSION_SECRET;
    if (process.env.NODE_ENV === "production") {
      throw new Error("AUTH_SESSION_SECRET is required in production");
    }
    return "dev-only-change-me";
  }

  private sign(value: string) {
    return createHmac("sha256", this.getSigningSecret()).update(value).digest("hex");
  }

  getCookieName() {
    return SESSION_COOKIE_NAME;
  }

  private encode(payload: AppSessionPayload) {
    const exp = Math.floor(this.now() / 1000) + SESSION_IDLE_SECONDS;
    const encoded = Buffer.from(JSON.stringify({ ...payload, exp })).toString("base64url");
    return `${encoded}.${this.sign(encoded)}`;
  }

  setCookie(response: NextResponse, payload: AppSessionPayload) {
    response.cookies.set(SESSION_COOKIE_NAME, this.encode(payload), {
      httpOnly: true,
      secure: this.isSecureCookie(),
      sameSite: "lax",
      path: "/",
      maxAge: SESSION_IDLE_SECONDS,
    });
  }

  /** Same cookie with the expiry moved 30 minutes ahead; call it whenever the user is seen to be active. */
  renewCookie(response: NextResponse, payload: AppSessionPayload) {
    this.setCookie(response, payload);
  }

  clearCookie(response: NextResponse) {
    response.cookies.delete(SESSION_COOKIE_NAME);
  }

  readFromCookieValue(cookieValue: string | undefined): AppSessionPayload | null {
    if (!cookieValue) return null;

    const [encoded, signature] = cookieValue.split(".");
    if (!encoded || !signature) return null;

    const expectedBuffer = Buffer.from(this.sign(encoded));
    const actualBuffer = Buffer.from(signature);
    if (expectedBuffer.length !== actualBuffer.length || !timingSafeEqual(expectedBuffer, actualBuffer)) {
      return null;
    }

    try {
      const payload = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
      if (!payload || typeof payload.userInv !== "string") return null;
      // A cookie without an expiry (issued before idle timeouts existed) is treated as expired.
      if (typeof payload.exp !== "number" || payload.exp * 1000 <= this.now()) return null;
      return {
        userInv: payload.userInv,
        empId: typeof payload.empId === "string" ? payload.empId : undefined,
      };
    } catch {
      return null;
    }
  }
}

export const authSessionService = new AuthSessionService();
